"""
Point cloud with SSAO (Screen Space Ambient Occlusion)
=====================================================

This example renders the Stanford Bunny point cloud (via Open3D) and applies
SSAO as a post-process to enhance ambient occlusion and depth perception.
"""
...
import pygfx as gfx
from pygfx.renderers.wgpu.engine.ssao import SSAOPass
from pygfx.renderers.wgpu.engine.blur import BilateralBlurPass
...
# Apply SSAO + bilateral blur as post-processing
renderer.effect_passes = [
    SSAOPass(radius=4.0, bias=0.015, intensity=1.3, power=1.0, normalize_depth=True),
    BilateralBlurPass(radius=2.0, sigma_spatial=2.0, sigma_depth=0.02),
]
